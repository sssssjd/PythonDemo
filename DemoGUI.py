...
def train_search():
    ...
    url3 = 'https://train.qunar.com/dict/open/s2s.do?dptStation='+q_star_city+'&arrStation='+q_arr_city+'&date='+date+'&type=normal&user=neibu&source=site&start=1&num=500&sort=3'
    ...
    try:
        ...
        for checi in lines:
            list = []
            list.append(checi['trainNo'] + ' ' + checi['dptStationName'] + '->' + checi['arrStationName'] + ' ' + checi[
                'dptTime'] + '-' + checi['arrTime'])
            b = checi['seats']
            for key, value in b.items():
                list.append(' %s,票价：%s,余票:%s张;' % (key, value['price'], value['count']))
