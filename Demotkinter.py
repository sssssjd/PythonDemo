...
def BMI(high,weigh):
    ...
    BMI = weigh / high / high * 10000

    if BMI < 18.5:
        return ("BMI=%.2f, 低于18.5，属于：过轻。" % BMI)
    elif 18.5 <= BMI < 25:
        return("BMI=%.2f, 位于18.5-25，属于：正常。" % BMI)
    elif 25 <= BMI < 28:
        return("BMI=%.2f, 位于25-28，属于：过重。" % BMI)
    elif 28 <= BMI < 32:
        return("BMI=%.2f, 位于28-32，属于：肥胖。" % BMI)
    elif BMI >= 28:
        return("BMI=%.2f, 高于32，属于：严重肥胖。" % BMI)
